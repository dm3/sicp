...
(deftest test-mul
  ...
  " incorrect, this version of mul (actually that's a responsibility of make-rat) can't deal with negative numbers "
  (is (= (mul-rat (make-rat -2 3) (make-rat 3 -4)) (make-rat -6 -12))))

; 2.1
; We'll define our own make-rat which will properly deal with negative numbers
; If the rational number is negative - only the numerator should be negative.
(defn stay-negative
